...
# when gesture_detector tells them to.
    gesture_detector.on_fist(
        lambda timestamp: gesture_lexer.add("fist", timestamp))
    gesture_detector.on_palm(
        lambda timestamp: gesture_lexer.add("palm", timestamp))
    gesture_detector.on_blink(
        lambda timestamp: gesture_lexer.add("blink", timestamp))
...
# strings passed into gesture_lexer.add in the anonymous lambdas above.
    gesture_parser.add_pattern(['fist', 'palm', 'fist'], lambda: print(
        "fist-palm-fist event has fired -- this message is not logged, only printed."))
...
# demonstration of them being used together shortly.
    ...
    # This line aggregates gestures into gesture sequences.
        gesture_sequences = gesture_lexer.lex(
            timestamp, min_increment, max_increment)
...
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    # an average EAR below the threshold value).
        gesture_detector.trigger_events(
            timestamp, open_eye_threshold, fist_perimeter, palm_perimeter, left_eye_perimeter, right_eye_perimeter)
